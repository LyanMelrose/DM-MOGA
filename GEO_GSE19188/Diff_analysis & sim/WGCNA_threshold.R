...
Mat<-readMat("_ENid_BP_GSE19188_Final.mat")
corMat<-Mat$after.PPI.Network
rm(Mat)
...
C_ENid <- read.table(
  "D:/_Documents/_Papers/Paper_3/DATA/GEO/GEO_GSE19188/GSE19188_M1_genes_ENTREZ.txt",
  header = FALSE,
  sep = ""
)
...
write.table(
  enrichGO_res@result,
  file = "D:/_Documents/_Papers/Paper_3/DATA/GEO/GEO_GSE19188/Diff_analysis & sim/GSE19188_M1_GO.txt",
  append = FALSE,
  quote = TRUE,
  sep = "\t",
  eol = "\n",
...
dotplot(enrichKEGG_res,showCategory=10,includeAll=TRUE)+scale_color_continuous(low='',high='')
# 在S3对象中，一般我使用$来访问一个对象的属性
# 但在S4对象中，我们只能使用@来访问一个对象的属性
write.table(
